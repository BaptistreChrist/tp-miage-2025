...
Baptiste CHRISTENSEN et Mathieu LEMAROUILLE
...
Il y a une couche base de données et code métier donc oui le code est structuré en couches.
Cependant il y a un problème de dépendance aux technos.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*
Le code métier est dans le fichier src/main/java/com/acmetodolist dans le TodoListController.

*Cette règle est-elle facilement testable par un test unitaire ?*
La règle est privée donc non elle n'est pas facilement testable il faut changer cela.
...
- docker run
--> lance un contener pour postgres on l’initialise avec le mot de passe POSTGRES_PASSWORD=password

- it
-->lance le contener en mode interaction

- e
--> représente la variable d’environement pour le contener
...
* Nous avons ajouté " this.todoItemRepository.save(todoItem)" à la méthode

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*
Dans TodoListController, le constructeur n'a pas de paramètres donc Spring ne peut pas l'instancier.
todoItemRepository n'est pas pris comme paramètre et n'est pas injecté donc reste null.
On peut le faire de manière "automatique" avec @Inject (@autowired dans ce cas comme Spring)
...
L’architecture hexagonale sépare la logique métier de l’infrastructure pour une meilleure organisation du code.
Ensuite, elle facilite l’interchangeabilité des composants grâce aux ports et adaptateurs.
Enfin, elle améliore la testabilité et l’évolutivité en isolant la logique métier.
...
- Un pour vérifier que un todo qui n'est pas en retard ne contient pas `[LATE!]`.
